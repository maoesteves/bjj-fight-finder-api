...
app.post('/buscar-lutas', async function(req, res) {
  ...
  try {
    ...
    for (var i = 0; i < linhas.length; i++) {
      ...
      if (/^[0-9]+$/.test(linha)) continue;
      if (/^[*]/.test(linha)) continue;

      if (/^[A-Za-z]/.test(linha) && linha.length >= 4) {
        for (var n = 0; n < names.length; n++) {
          if (corresponde(names[n], linha)) {
            ...
            if (!jaTem) {
              lutas.push({ athlete_name: linha, mat: matAtual || '-' });
            }
            break;
          }
        }
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', servidor: 'BJJ Fight Finder - v27' });
});

app.listen(PORT, '0.0.0.0', function() {
  console.log('BJJ Fight Finder v27 rodando na porta ' + PORT);
});
